...
const Insights: React.FC<InsightsProps> = ({ localData }) => {
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      {/* Summary Card */}
      <div className={`p-4 sm:p-6 rounded-lg border-2 ${hasDeviations ? 'bg-amber-50 border-amber-200' : 'bg-nudge-cream border-nudge-brown-light'}`}>
        <div className="flex items-start space-x-3">
          {hasDeviations ? (
            <AlertTriangle className="h-5 w-5 sm:h-6 sm:w-6 text-nudge-brown mt-1 flex-shrink-0" />
          ) : (
            <CheckCircle className="h-5 w-5 sm:h-6 sm:w-6 text-nudge-brown-light mt-1 flex-shrink-0" />
          )}
          <div className="flex-1 min-w-0">
            <h2 className="text-base sm:text-lg font-semibold text-nudge-slate">
              {hasDeviations ? 'Deviations Detected' : 'No Significant Deviations'}
            </h2>
            <p className="mt-1 text-xs sm:text-sm text-nudge-gray">
              {hasDeviations 
      ...
      {/* Thresholds Info */}
      <div className="bg-nudge-background rounded-lg p-3 sm:p-4">
        <div className="flex items-center space-x-2 mb-2 sm:mb-3">
          <Info className="h-4 w-4 sm:h-5 sm:w-5 text-nudge-gray flex-shrink-0" />
          <h3 className="text-xs sm:text-sm font-semibold text-nudge-slate">Deviation Thresholds</h3>
        </div>
        <div className="space-y-2 text-xs sm:text-sm text-nudge-gray">
          <div className="flex items-start space-x-2">
            <div className="w-2 h-2 bg-nudge-brown rounded-full flex-shrink-0 mt-1"></div>
            <span><strong>Incidence Rate:</strong> Deviation {'>'} 10 percentage points from global average</span>
          </div>
          <div className="flex items-start space-x-2">
            <div className="w-2 h-2 bg-nudge-brown-light rounded-full flex-shrink-0 mt-1"></div>
      ...
      {incidenceDeviations.length > 0 && (
        <div className="bg-white rounded-lg shadow p-4 sm:p-6">
          <div className="flex items-center space-x-2 mb-3 sm:mb-4">
            <TrendingUp className="h-4 w-4 sm:h-5 sm:w-5 text-nudge-brown flex-shrink-0" />
            <h3 className="text-base sm:text-lg font-semibold text-gray-900">Incidence Rate Aberrations</h3>
          </div>
          <div className="space-y-3">
            {incidenceDeviations.map(dev => (
              <div key={dev.ageGroup} className={`p-3 sm:p-4 rounded-lg border ${getDeviationBgColor(dev.difference)}`}>
                <div className="flex flex-col sm:flex-row sm:justify-between gap-3">
                  <div className="flex-1">
                    <h4 className="text-sm sm:text-base font-semibold text-gray-900">Age Group {dev.ageGroup}</h4>
                  ...
                  <div className="text-left sm:text-right">
                    <p className={`text-xl sm:text-2xl font-bold ${getDeviationColor(dev.difference)}`}>
                ...
                <div className="mt-3 pt-3 border-t border-gray-200">
                  <p className="text-xs sm:text-sm text-nudge-slate">
                    {dev.difference > 0 
      ...
      {diopterDeviations.length > 0 && (
        <div className="bg-white rounded-lg shadow p-4 sm:p-6">
          <div className="flex items-center space-x-2 mb-3 sm:mb-4">
            <TrendingUp className="h-4 w-4 sm:h-5 sm:w-5 text-nudge-brown-light flex-shrink-0" />
            <h3 className="text-base sm:text-lg font-semibold text-gray-900">Diopter Strength Deviations</h3>
          </div>
          <div className="space-y-3">
      ...
      {!hasDeviations && (
        <div className="bg-white rounded-lg shadow p-6 sm:p-8 text-center">
          <CheckCircle className="h-10 w-10 sm:h-12 sm:w-12 text-nudge-brown-light mx-auto mb-3 sm:mb-4" />
          <h3 className="text-base sm:text-lg font-semibold text-gray-900 mb-2">All Metrics Within Normal Range</h3>
          <p className="text-xs sm:text-sm text-nudge-gray max-w-2xl mx-auto">
      ...
      {hasDeviations && (
        <div className="bg-white rounded-lg shadow p-4 sm:p-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-start space-x-3">
              <div className="w-2 h-2 bg-gray-500 rounded-full mt-1 sm:mt-2 flex-shrink-0"></div>
              <div className="flex-1">
